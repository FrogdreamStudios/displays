...
public class Settings {
    ...
    private static final Map<String, Map<UUID, FullDisplayData>> serverDisplays = new HashMap<>();
    ...
    public static void load() {
        ...
        File clientSettingsFile = new File(SETTINGS_DIR, "client-display-settings.json");
    ...
    public static void loadServerDisplays(String serverId) {
        ...
        File serverFile = new File(SETTINGS_DIR, "server-" + serverId + "-displays.json");

        try (Reader reader = new FileReader(serverFile)) {
            Type type = new TypeToken<Map<String, FullDisplayData>>() {
            }.getType();
            Map<String, FullDisplayData> loadedDisplays = GSON.fromJson(reader, type);

            Map<UUID, FullDisplayData> displays = new HashMap<>();
            if (loadedDisplays != null) {
                for (Map.Entry<String, FullDisplayData> entry : loadedDisplays.entrySet()) {
                    try {
                        UUID uuid = UUID.fromString(entry.getKey());
                        displays.put(uuid, entry.getValue());
                    } catch (IllegalArgumentException e) {
                        LoggingManager.error("Invalid UUID in server displays: " + entry.getKey());
                    }
                }
            }
    ...
    public static void saveServerDisplays(String serverId) {
        try {
            ...
            Map<UUID, FullDisplayData> displays = serverDisplays.getOrDefault(serverId, new HashMap<>());
            ...
            LoggingManager.info("Saved " + toSave.size() + " displays for server: " + serverId);
        } catch (IOException e) {
    ...
    public static @Nullable FullDisplayData getDisplayData(UUID displayId) {
        ...
        return serverDisplays.getOrDefault(currentServerId, new HashMap<>()).get(displayId);
    }
    ...
    public static void saveDisplayData(UUID displayId, FullDisplayData data) {
        ...
        serverDisplays.computeIfAbsent(currentServerId, k -> new HashMap<>()).put(displayId, data);
    ...
    public static void removeDisplay(UUID displayId) {
        ...
        for (Map<UUID, FullDisplayData> displays : serverDisplays.values()) {
            if (displays.remove(displayId) != null) {
                ...
                for (Map.Entry<String, Map<UUID, FullDisplayData>> entry : serverDisplays.entrySet()) {
        ...
        LoggingManager.info("Removed display from all saved data: " + displayId);
    }
    ...
    public static class FullDisplayData {
        ...
        public FullDisplayData(UUID id, int x, int y, int z, String facing, int width, int height,
                               String videoUrl, String lang, float volume, String quality, boolean muted,
                               boolean isSync, UUID ownerId, int renderDistance, long currentTimeNanos) {
