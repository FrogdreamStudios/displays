...
fun upload(
    ...
    val forceCommandEncoder = renderer != null && renderer != "vulkan_zink"
    if (texture is GlTexture && RenderBackendCompat.canUseDirectOpenGl() && !forceCommandEncoder) {
...
fun uploadPlanar(
    ...
    val renderer = System.getenv("POJAV_RENDERER")
    val forceCommandEncoder = renderer != null && renderer != "vulkan_zink"
    if (y is GlTexture && u is GlTexture && v is GlTexture && RenderBackendCompat.canUseDirectOpenGl() && !forceCommandEncoder) {
...
private fun writeToTexture(
    ...
    width: Int,
    height: Int,
    @Suppress("UNUSED_PARAMETER") format: NativeImage.Format,
) {
    //? if >=1.21.11 {
    //? if >=26.2 {
    val encoder = RenderSystem.getDevice().createCommandEncoder()
    encoder.writeToTexture(texture, buffer, 0, 0, 0, 0, width, height)
    encoder.submit()
    //?} else
    /*val encoder = RenderSystem.getDevice().createCommandEncoder()
    ...
    try {
        encoderClass.getMethod(
            ...
            Int::class.javaPrimitiveType,
            Int::class.javaPrimitiveType
        ).invoke(encoder, texture, buffer, format, 0, 0, width, height, 0, 0)
        ...
        encoderClass.getMethod(
            ...
            Int::class.javaPrimitiveType,
            Int::class.javaPrimitiveType
        ).invoke(encoder, texture, buffer, 0, 0, width, height, 0, 0)
    }
    encoder.submit()*/
    //?}
}
