...
/** `FFmpeg` binary downloader / resolver. **/
object FFmpegBinary {
    ...
    /**
     * On Android the binary ships inside the Pojav ffmpeg plugin APK instead of being downloaded
     * (see [AndroidPluginLibs]); everywhere else, checks the cache directory for an existing binary,
     * downloads and extracts one if not found, and falls back to the system `FFmpeg` on any failure.
     */
    private fun resolve(): String? {
        AndroidPluginLibs.ffmpegPath()?.let { path ->
            logger.info("Using Android ffmpeg plugin binary: $path.")
            return path
        }
