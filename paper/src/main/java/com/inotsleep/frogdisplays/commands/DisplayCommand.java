package com.inotsleep.frogdisplays.commands;
...
import com.inotsleep.frogdisplays.listeners.SelectionListener;
...
public class DisplayCommand extends AbstractCommand {
    ...
    private void handleVideoCommand(CommandSender sender, String[] args) {
        if (args[0].equals("video")) {
            ...
            if (block.getType() != FrogdisplaysPlugin.config.settings.baseMaterial) {
                MessageUtil.sendColoredMessage(player, FrogdisplaysPlugin.config.messages.noDisplay);
                return;
            }
            ...
            if (code == null) {
                MessageUtil.sendColoredMessage(player, FrogdisplaysPlugin.config.messages.invalidURL);
                return;
            }
            ...
            MessageUtil.sendColoredMessage(player, FrogdisplaysPlugin.config.messages.settedURL);
        }
    }
    ...
    private void handle1Args(CommandSender sender, String[] args) {
        switch (args[0]) {
            case "create" -> {
                ...
                MessageUtil.sendColoredMessage(player, FrogdisplaysPlugin.config.messages.successfulCreation);
            }
            case "delete" -> {
                if (!(sender instanceof Player player)) return;
                if (!player.hasPermission(FrogdisplaysPlugin.config.permissions.deletePermission)) {
                    return;
                }
            ...
            case "reload" -> {
                if (!sender.hasPermission("frogdisplays.reload")) {
                    sendHelp(sender);
                    return;
                }

                FrogdisplaysPlugin.config.reload();
                MessageUtil.sendColoredMessage(sender, "&aFrog Display has been reloaded!");
            }
            case "list" -> {
                if (!sender.hasPermission(FrogdisplaysPlugin.config.permissions.listPermission)) {
                    sendHelp(sender);
                    return;
                }
                ...
                MessageUtil.sendColoredMessages(
                        sender,
                        FrogdisplaysPlugin.config.messages.usageHeader
                                .stream()
                                .map(
                ...
                usage.forEach(
                        (k, v) -> MessageUtil.sendColoredMessage(
                                ...
                                me.inotsleep.utils.MessageUtil.parsePlaceholders(
                                        FrogdisplaysPlugin.config.messages.usageVersionEntry,
                                        k.toString(),
                                        String.valueOf(v)
                                )
                        )
                ...
                MessageUtil.sendColoredMessage(sender, FrogdisplaysPlugin.config.messages.usageFooter);
            }
        }
    }
    ...
    public List<String> complete(CommandSender sender, String[] args) {
        if (args.length == 1) {
            ...
            if (sender.hasPermission(FrogdisplaysPlugin.config.permissions.deletePermission)) completions.add("delete");
            if (sender.hasPermission(FrogdisplaysPlugin.config.permissions.listPermission)) completions.add("list");
            return completions;
        }
        return List.of();
    }
