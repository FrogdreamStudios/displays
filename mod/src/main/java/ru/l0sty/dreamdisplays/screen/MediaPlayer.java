...
public class MediaPlayer {
    ...
    // === CONSTANTS =======================================================================
    // Поддерживаем WebM и MP4 контейнеры
    ...
    private void initialize() {
        try {
            ...
            availableVideoStreams = all.stream()
                    .filter(s -> MIME_VIDEO.equals(s.getMimeType()) || MIME_VIDEO_MP4.equals(s.getMimeType()))
                    .toList();
            ...
            Optional<Stream> audioOpt = audioS.stream()
                    .filter(s -> MIME_AUDIO.equals(s.getMimeType()) || MIME_AUDIO_MP4.equals(s.getMimeType()))
            ...
            if (audioOpt.isEmpty()) {
                ...
                ///  If no audio stream with the specified language is found,
                ///  fallback to the last audio stream available.

                // TODO: but what if 403 error occurs?
                audioOpt = all.stream()
    ...
    private Pipeline buildAudioPipeline(String uri) {
        String desc = "souphttpsrc location=\"" +
                uri + "\" proxy=\"http://127.0.0.1:14881\"" +
                "! decodebin" +
                "! audioconvert" +
                "! audioresample " +
