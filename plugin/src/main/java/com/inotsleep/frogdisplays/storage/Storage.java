package com.inotsleep.frogdisplays.storage;
...
public class Storage {
    ...
    public Storage(FrogdisplaysPlugin plugin) {
        this.plugin = plugin;

        new BukkitRunnable() {
            public void run() {
                tablePrefix = FrogdisplaysPlugin.config.storageSettings.tablePrefix;

                try {
                    connection = BaseConnection.createConnection(FrogdisplaysPlugin.config.storageSettings, plugin.getDataFolder());
                    connection.connect();

                    onConnect();
                } catch (SQLException e) {
                    LoggingManager.error("Could not connect to database", e);
                    FrogdisplaysPlugin.disablePlugin();
                }
            }
